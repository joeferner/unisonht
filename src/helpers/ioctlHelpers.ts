...
export async function readIoctl8(fd: fs.promises.FileHandle, cmd: number): Promise<number> {
  const v = Buffer.alloc(1);
  try {
    if (ioctl(fd.fd, cmd, v) !== 0) {
      throw new Error("failed to ioctl");
    }
  } catch (err) {
...
export async function writeIoctl8(fd: fs.promises.FileHandle, cmd: number, value: number): Promise<void> {
